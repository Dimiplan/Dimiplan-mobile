...
class _AddTaskScreenState extends State<AddTaskScreen> {
  ...
  final TextEditingController _newPlannerNameController = TextEditingController();
  ...
  Future<void> _loadPlanners() async {
    ...
    try {
      ...
      if (_planners.isNotEmpty && widget.task == null && widget.selectedPlannerId == null) {
        setState(() {
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          actions: <Widget>[
            ...
            TextButton(
              child: Text('추가'),
              onPressed: _isAddingPlanner ? null : () async {
                              ...
                              child: Column(
                                children: [
                                  DropdownButtonFormField<int>(
                                    ...
                                    icon: Icon(Icons.arrow_drop_down_circle),
                                    iconSize: 22.0,
                                    items: _planners.map((Planner planner) {
                                      return DropdownMenuItem(
                                        value: planner.id,
                                        child: Text(
                                          planner.name,
                                    ...
                                    value: _planners.isNotEmpty ? 
                                           (_planners.any((p) => p.id == _from) ? _from : _planners.first.id) : null,
                                  ),
                                  SizedBox(height: 8),
                                  TextButton.icon(
                                    onPressed: () {
